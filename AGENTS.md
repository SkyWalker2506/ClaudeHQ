...
> Tum genel kurallar `~/Projects/Codex-config/AGENTS.md` dosyasindadir ve burada da gecerlidir.

## Bu proje nedir

ClaudeHQ, Musab Kara'nin Codex ekosisteminin merkezi calisma alanidir. Buradan tum projelere erisebilir, cross-project gorevler yonetebilirsin.

## Oturum basinda

1. `~/Projects/Codex-config/AGENTS.md` oku ve uygula
...
- Her projenin kendi AGENTS.md kurallari o proje icinde gecerlidir
...
ClaudeHQ, tum projeleri merkezi olarak yonetir. `~/Projects` altinda AGENTS.md olan her klasor otomatik proje olarak taninir.
...
- `./scripts/hq new <isim> [--jira KEY]` — Yeni proje olustur (git, AGENTS.md, sprint dahil)
- `./scripts/hq session <proje>` — Tek proje icin Codex session baslat
...
Once projeleri kesfet veya yeni proje olustur, ardindan sprint baslat ve task'lari tanimla. Dispatch ile Codex session'larini calistir, monitor ile izle, takilan varsa mudahale et. Tum komutlar yukaridaki "Komutlar" bolumunde listelenmistir.
...
|------------|---------------------|
| Agent eklendi/silindi (`Codex-config/agents/`) | `Codex-config/README.md`, `Codex-agent-catalog/README.md`, `ClaudeHQ/README.md` |
| Plugin eklendi/silindi (`ccplugin-*`) | `Codex-config/README.md`, `Codex-marketplace/README.md`, `marketplace.json`, `ClaudeHQ/README.md` |
| Plugin marketplace.json'a eklendi | `Codex-marketplace/README.md` (tablo), `marketplace.json` (description) |
| Skill eklendi/silindi (`Codex-config/global/skills/`) | (sayilar henuz README'lerde yok, eklenirse guncelle) |
...
2. `cd ~/Projects/Codex-config && ./install.sh`
